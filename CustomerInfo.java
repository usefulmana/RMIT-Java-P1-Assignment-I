...
public class CustomerInfo
{
    ...
    public CustomerInfo(String name, String birthday, String address, String phone, String email) {
        this.name = name;
    ...
    private String formatStr = "%-7s %-22s %-12s %-45s %-20s %-15s%n"; // Use this to format text file
    ...
    {   // This method is created to check if a line of info exists in the text file or not
        String line = null;
        try
        {
            BufferedReader bufferedReader = new BufferedReader(new FileReader(customerFileName));
            ...
            while ((line = bufferedReader.readLine()) != null)
            {
                ...
                {
                    return true; // Return True if it contains user input
                }
            }
            bufferedReader.close();
    ...
    public void editFile(String FileName, String lineHexa, String tobeReplaced, String toReplace)
    {   // This method is used to edit individual information on each line in a text file
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (!tempArr[0].equals(lineHexa)) // Use the Hexadecimal number to fine the line that is needed
                {
                    oldtext += line + "\n"; // Add non-target lines to a string
                }
                else
                {
                    toBeReplaced += line; // Add target line to a different string
                }
            }
            // Modifying target line
            String newStr = toBeReplaced.replace(tobeReplaced, toReplace);

            // Add the modified target line to the other lines
            oldtext += newStr + "\n";

            // Create a dummy textfile
            File inputFile = new File(FileName);
            File tempFile = new File("tempFile.csv");
            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(tempFile, true));

            // Write
            bufferedWriter.write(oldtext);
    ...
    public boolean infoChecker(String FileName, String userInput) throws FileNotFoundException
    {   // This method is used to check if the info an user enters is in the target line
        try {
            // Importing reader
            BufferedReader bufferedReader = new BufferedReader(new FileReader(FileName));
            String line = null;
            while ((line = bufferedReader.readLine()) != null)
            {
                ...
                {
                    return true; // True if the line contains user input
                }
            }
            bufferedReader.close();
        }
